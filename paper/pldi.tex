...
While the statistics present quantitative measurements for a variety
of examples, the importance of qualitative measurements should not be
overlooked. Our experience with developing in Ynot have evolved both
the tactics and the style of development. The first parameter to {\tt
  sep} which performs definition unfolding was added specifically to
allow, previously common, assertions for unfolding definitions to be
eliminated. This requires relatively mechanical lemmas to be proved
about definition unfolding, but developing without annotations has
made modifying code much easier since annotations need not be kept in
sync with variable names or the order of statements. Despite the
occasionally, seemingly, obvious implications which automation fails
to detect, development efforts are able to proceed quickly and
automation is very much ``pay as you go.'' The \todo{binomial tree}
sketching code with {\tt Assert} statements and then removing them
though automation works well and, since tactics are reused, subsequent
developments are faster and require fewer annotations during
prototyping.
...
using specification variables and pre-post conditions.  The systems 
...
the low-level language of set theory, Ynot provides a separation logic layer that, 
coupled with Coq, lets programmers build their own abstractions.  
...
Isabelle and Coq, and also the MONA prover for the monadic fragment of 2nd order logic.  In contrast, Ynot's automated reasoning is implemented completely in Coq.  Our approach is extremely lightweight: the entire Ynot codebase is smaller than just the Jahob-MONA interface.
