...
Sage~\cite{sage}, ATS~\cite{ats}, Concoqtion~\cite{concoqtion}, and
...
is too weak to support full verification because these systems rely upon
provers to automatically discharge verification conditions.  While
there have been great advances in the performance of automated
provers, in practice, they can can only handle relatively shallow
fragments of a first-order logic.  Thus, programmers are frustrated when code
that is correct is rejected by the type-checker.  For example, none
of these systems is able to prove that an array index is bounds when
the constraints involved step outside Pressburger arithmetic.  

In contrast, ATS, Concoqtion and Ynot use a powerful, higher-order
logic that supports a much wider range of policies including (partial)
correctness.  Furthermore, in the case of Ynot, programmers can define
and use connectives in the style of Separation Logic~\cite{sep-logic}
...
type-checker that the code is correct.  Unfortunately, explicit proofs
can be quite large when compared to the code: For example, in the Ynot
...
assistant~\cite{coq}.  This allows us to inherit the full power of
Coq's dependent types for writing code, specifications, and proofs and
to extract executable ML code.  However, unlike the previous version,
we have taken advantage of Coq's tactic language Ltac to
implement a new set of parameterized semi-decision procedures for
automatically discharging or at least simplifying the separation logic
style verification conditions.  The careful design of these procedures
makes it possible for programmers to teach the prover about new domains
as they arise.  
...
structures including stacks, queues, hash tables, binary heaps, and
...
most cases, programmers need only prove key lemmas regarding the
abstractions used in their interfaces, and plug these lemmas into our
extensible tactics to realize automation.  More importantly, we show
...
Jahob~\cite{jahob} and Smallfoot~\cite{cite}.  To summarize, Jahob is
based on the ideas of ESC-Java and Spec\# but aims for full
...
decision procedures, some of which do not provide witnesses.
...
existing abstractions (linked-list segments, and tree-segments) and
thus will not scale to new abstractions.  
