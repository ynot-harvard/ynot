...
compltely automatic reasoning and concurrency, and it's almost
complete nature makes many correct programs go through with only the
function level annotations. Since the current version of Ynot does not
address concurrency (it is planned for the future), we will not
address this in our comparison. Based on these points, we draw a
comparison in two parts: the size of annotations and proof obligations
and the expressivity of the logic.
