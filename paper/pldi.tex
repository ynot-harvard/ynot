...
module implementing a enqueue for a linked-list, requires 7 lines of code,
...
Direct reasoning about heaps leads to very cumbersome proof obligations, with many sub-proofs that pairs of pointers are not equal.  Separation logic~\cite{separation} is the standard tool for reducing that complexity.  The previous Ynot system built a separation logic on top of the axiomatic foundation, and we do the same here.  As before, we introduce no new syntactic class of separation logic formulas.  Instead, we define functions that operate on arbitrary predicates over heaps, with the intention that we will only apply them on separation-style formulas.  Nonetheless, it can be helpful to think of our ``assertion language'' as defined by:
$$\begin{array}{rcl}
...
standard separation logic.  The old Ynot separation logic included
...
p --> x * q --> y ==> ?123 x
\end{verbatim}
we set \cd{?123} to \cd{fun x' => p --> x' * q --> y}.
...
When \cd{sep} has run out of rules to apply, the remaining subgoal is subjected to standard Coq automation.  Propositional structure and calls to recursive functions are simplified where possible.  \cd{sep} ends by running a loop over those simplifications and the simplifications performed by the user's \cd{solver} tactic, until no further progress can be made.  Finally, \cd{sep} discharges every remaining goal of the form \cd{P ==> P}, by reflexivity.

Every step of the overall process is implemented in Ltac, so that only a bug in Coq would allow \cd{sep} to declare an untrue goal as true, no matter which customization the programmer provides.  By construction, every step builds an explicit proof term, and we can validate all proof terms afterward with an independent checker that is relatively simple, compared to the operation of all of the decision procedures that may have contributed to the proof.
...
We have implemented several data structures using a variety of techniques.  The Ref to Functional Finite Map module implements a finite map by storing a pointer to a pure functional finite map in the heap.  The Stack and Queue are fully automated examples built from the ground up using the principles in this paper.  The Hashtable and Binary Search Tree implement a dependently typed (i.e., heterogenous) finite map and are adapted from the previous version of Ynot; we also include statistics for the older versions.  The hashtable is the only example to use arrays (todo check).  The binomial tree uses strong heap update to merge trees and exploits dependent types to compute an equivalent pure functional binary tree using a terminating Coq fixpoint.  The last two examples are the Ynot equivalents of examples from two related systems which are discussed later.

Our experience developing Ynot has resulted in hard won
practical lessons about proof engineering and Coq pragmatics.
Our Coq tactics and development methodology are directly
based on this experience.  For instance, the first parameter to {\tt
  sep}, which performs definition unfolding, was specifically added to
abstract a common, natural design pattern.  

This automation requires the programmer to prove relatively
mechanical lemmas about definition unfolding.  Instead,
a ynot programmer may use explicit framing and assertion annotations.  
Even at the level of our examples, we have found that annotations tend to
hinder reusability.  However, they are indispensible
for the occasional, seemingly obvious heap implications 
which automation fails to detect.  In addition, it is sometimes convenient
to separate the programming 
and automation aspects of development, and
sketching code with assertions to be removed later
lets Ynot programmers do exactly that.  

Figure \ref{loc} presents code size statistics for the case studies that we have completed in our new system.  Ynot modules have a particular structure which is reflected in the column headings: specifications, heap representation, proofs, tactics, and program code.  Ynot modules
begin with two sections that we ignore in our statistics: first, an ADT module type is defined, and then a pure functional model, 
including proofs and tactics, is defined or imported.  We ignore the module type definitions because they would be double counted 
in the specifications column, which includes the pre and post conditions of every function defined in the module.  Because some 
equivalent of a functional model is required for verification in any system, and functional models vary widely in their state of 
development, we do not include such functional code in the statistics.  The core of a ynot module then consists of heap representation
 code (e.g. {\texttt rep}), and proofs (e.g. {\texttt push\_list}) and tactics (e.g. {\texttt simp\_prem}) about the representation; 
 these three columns, plus the annotations column, are totaled in the overhead column.  The annotations column counts the number of lines of
 programmer specified annotations (e.g. \texttt{<@>}). Program code (e.g. getElements) is code that is preserved by extraction. 
  Coqc compilation times (in minutes:seconds) were taken on a 2.8ghz Pentium D 
 with 1gb of memory.   We have made no effort to optimize our tactics for running time.  We have also made no effort to share overhead code between related examples. 
...
Our examples indicate that Ynot compiles quickly enough to be useful in practice, and that a variety of tradeoffs between columns are possible.  Development is done interactively with Coq, so programmers are typically only exposed to shorter, single method verification times.  The linked list segments example suggests that our techniques can be made to scale linearly.   To qualitatively evaluate Ynot we have compared it against two 
similar, related systems, Jahob~\cite{jahob} and Smallfoot, by implementing in Ynot
one example from each other system.
...
A slight modification of Coq is required for Ynot 
to be consistent with the law of the 
excluded middle (and proof irrelevence).   
...
We have compared the Jahob association list example with an equivalent in Ynot program.  
The Ynot program is X larger, but it not only contains functionality that is implemented by
the Jahob backend, it also contains reasoning code that
can presumably be shared with similar datastructures.  Modulo these differences,  
the Jahob and Ynot examples are, perhaps unsurprisingly, quite similar.  For instance,
  they both require large specifications for loop invariants --  
in the remove method, for instance, Jahob uses X lines of such specs and Ynot
uses Y lines.  Our Ynot implementation uses explicit framing conditions in 
places where Jahob does not, although there is no reason that we know of that these
annotations could not be inferred using the techniques in this paper. 
...
\noindent The definition is expressible in the same manner as the
...
guarantee non-cycles.

For comparing annotatations, we consider {\tt append} function on
lists as a representative example. In Smallfoot, the function is built
using a {\tt while} loop, requires 14 lines, and three annotations:
the pre- and post-conditions and the loop invariant. To maintain a
close symmetry between the definitions, we break the {\it append}
function into two pieces, one for the loop and the other for the main
body. The two functions are, respectively, 4 and 5 lines long and
require 2 lines of annotations for the fixpoint definition which
corresponds to the Smallfoot loop invariant. As with all previous
examples, the proof-automation tactics and lemmas used for this
function apply to all functions dealing with linked lists in the same
way that the linked list representation predicate is axiomatized in
the Smallfoot system and is reused.
...
dependent types to represent partial correctness while Smallfoot
guarantees only heap-safety. This is captured in the type of the {\it
  cons} function:
\begin{verbatim}
cons :: forall (v : A) (r : LL) (q : [LL])
    (m : [list A]) (T : Type) (vt : [T]),
  STsep (q ~~ m ~~ vt ~~ llseg r q m * q ~~> vt)
        (fun r:LL => q ~~ m ~~ vt ~~ 
                  llseg r q (v :: m) * q ~~> vt).
\end{verbatim}
In Smallfoot, the system can guarantee only that the returned list is
valid. Thus, {\it cons}, {\it removeFirst}, etc. all have the same
type despite the fact that they do different things.

The second advantage in Ynot is the ability to deal use higher-order
logic to express properties. The ease of use in Smallfoot comes at the
price of restricting programs to a small set of data structures
(including linked lists, doubly linked lists, trees, and xor-lists)
for which the definitions and corresponding inductive properties can
be defined using predicate logic. The theory built for each data
structure in Smallfoot plays the same roll as the unrolling lemmas for
which counts are given in Figure \ref{loc}.
