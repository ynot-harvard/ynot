...
We have implemented several data structures using a variety of techniques.  The Ref to Functional Finite Map module implements a finite map by storing a pointer to a pure functional finite map in the heap.  The Stack and Queue are fully automated examples built from the ground up using the principles in this paper.  The Hashtable and Binary Search Tree implement a dependently typed (i.e., the type of a value depends on its key) finite map and are adapted from the previous version of Ynot; the hashtable uses arrays (todo check).  The binomial tree has no proofs, uses strong heap update to merge trees and exploits dependent types to compute an equivalent pure functional binary tree using a terminating Coq fixpoint.  The last two examples were used in comparisons with tools similar to Ynot and are discussed later.
...
Of course, using a pattern requires overhead, and
for us this overhead is the need to prove 
relatively mechanical lemmas 
about definition unfolding.  Ynot aims to be a pay as you go
system, and Ynot programs can 
use assertions rather than integrate with the
sep tactic.  Assertions are also indispensible
for the occasional, seemingly obvious implications 
which automation fails to detect.  Even at the 
level of our examples, we have found that assertions
hinder reusability.  Nevertheless, it is sometimes convenient
...
Figure \ref{loc} presents code size statistics for the case studies that we have completed in our new system.  Each Ynot module begins with ADT specifications.  This plumbing code is not included in the statistics.  Each module 
then defines or imports a pure functional model, which is also not counted in the statistics.

The column ``Program'' counts the number of lines that have equivalents in extracted ML programs, .  ``Total Non-Program'' gives the sum of all columns that follow it.  ``Specs'' includes type signatures and pre- and postconditions for the methods.  ``Annotations'' includes additional annotations within function definitions, included only for proof purposes.  ``Aux. Defs.'' covers auxiliary definitions of notions like representation predicates.  ``Proofs'' includes statements and proofs of example-specific lemmas, and ``Tactics'' includes example-specific coding of Ltac semi-decision procedures.  The count does not include code for pure functional models that are often shared between modules - for instance, the association list model contains 15 lines of aux defs, 35 lines of proof, and 15 lines of tactics.  Coqc compilation times (in minutes:seconds) were taken on a 2.8ghz Pentium D with 1gb of memory.  We have made no effort to optimize our tactics for running time.

To qualitatively evaluate Ynot we have compared it against two 
similar, related systems, Jahob~\cite{jahob} and Smallfoot, by implementing in Ynot
one example from the other system.
...
Ref to Functional Finite Map                 &   8     &    19             &  15   &    0        &  0         &  2     & 2       & 0:05 \\
Hash Table                                   &  28     &   234             &  29   &   28        & 23         & 103    &  51     & 0:45 \\
Hash Table (old) \\
BST Finite Map                               &  17     &   160             &  18   &    4        & 51         & 63     & 24      & 1:35 \\
BST Finite Map  (old)                        &  17     &   160             &  18   &    4        & 51         & 63     & 24      & 1:35 \\
Binomial Tree                                &  17     &    44             &  14   &    7        & 13         &  0     & 10      & 2:33 \\
Association List$^1$           &  43     &   174             &  33   &   14        & 16         & 63     & 42      & 3:10 \\
Linked List$^2$                &  84     &   421             &  36   &    8        & 20         & 127    & 230     &  tbd \\
    \end{tabular}
  \end{center}
\footnotesize
$^1$ Jahob comparison, $^2$ Smallfoot comparison
\normalsize
