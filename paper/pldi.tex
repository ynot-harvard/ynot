...
fragments of first-order logic.  Thus, programmers are frustrated when code
that is correct is rejected by the type-checker.  For example, none
of these systems is able to prove that an array index is in bounds when
the constraints involved step outside quantifier-free linear arithmetic.  
...
type-checker that code is correct.  Unfortunately, explicit proofs
...
automatically discharging or at least simplifying the separation
logic-style verification conditions.  The careful design of these procedures
